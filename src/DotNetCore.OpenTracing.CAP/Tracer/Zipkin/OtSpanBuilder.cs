...
namespace OpenTracing.Contrib.CAP.Tracer.Zipkin
{
    internal class OtSpanBuilder : ISpanBuilder
    {
        ...
        public ISpanBuilder AsFollowsFrom(ISpan parent)
        {
        ...
        {
            if (_tags == null || !_tags.TryGetValue(Tags.SpanKind.Key, out var spanKind))
            {
            ...
            switch (spanKind)
            {
                case Tags.SpanKindClient:
                    return OtSpanKind.Client;
                case Tags.SpanKindServer:
                    return OtSpanKind.Server;
                case Tags.SpanKindProducer:
                    return OtSpanKind.Producer;
                case Tags.SpanKindConsumer:
                    return OtSpanKind.Consumer;
                default:
                    return OtSpanKind.Local;
            }
        }
